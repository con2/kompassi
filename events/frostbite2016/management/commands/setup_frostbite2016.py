...
class Setup(object):
    ...
    def setup_labour(self):
        ...
        for name, description, pcs in [
            ...
            ('Narikka', 'Narikkavänkärit ovat vastuussa tapahtuman narikoiden pyörittämisestä. Työ itsessään on yksinkertaista, mutta tekemistä sen sijaan on varmasti riittävästi. Narikkavänkärit ovat määrällisesti suurin vänkäriryhmä.', [tyovoima, ylivankari]),
            ('Siivous', 'Siivousvänkärit ovat vastuussa tapahtuman yleisestä siisteydestä. He kulkevat ympäriinsä tehtävänään roskakorien tyhjennys, vesipisteiden täyttö, vessoihin papereiden lisääminen ja monet muut pienet askareet. Työ tehdään pääsääntöisesti vänkäripareittain.', [tyovoima, ylivankari]),
            ('Green Room', 'GreenRoom -vänkärinä sinun täytyy olla kykeneväinen itsenäiseen ja oma-aloitteeseen työskentelyyn. Tehtäviisi kuuluu kahvin keitto, esivalmistelutyöt, paikkojen siistiminen ym. Huomioithan että tekemistä riittää GreenRoomissa lähestulkoon läpi vuorokauden, joten tämä ei ole pesti jossa pärjää peukkujen pyörittelyllä.', [tyovoima, ylivankari]),
            ...
            ('Valokuvaaja', 'Desukuvaajana tehtävänäsi on yleis-, ohjelma- ja photoshoot-kuvaus. Kuvauspisteen kuvaajat kuvaavat cosplayasuja studiossa. Valokuvaus tapahtuu kuvaajien omalla kalustolla.', [tyovoima, ylivankari]),
            ('Taltiointi', 'Taltiointivänkärit tallentavat Desuconin ohjelmista äänen ja kuvan. Osa taltiointitiimin jäsenistä myös editoi tämän materiaalin, jotta ohjelmat ovat nähtävissä myöhemminkin. Lisäksi taltiointivänkärit kuvaavat videomateriaalia muuten tapahtumasta.', [tyovoima, ylivankari]),
            ('Myyntisali', 'Myyntisalivänkärin tehtävänä on pääsääntöisesti vastata Desuconin oman oheistuotemyyntipöydän tuotteiden järjestelemisestä ja myymisestä. Myyntisalivänkärin toimenkuvaan kuuluu myös myyntisalissa päivystäminen: tämä tarkoittaa kävijöiden kysymyksiin vastaamista, ohjeistamista, myyjien Green Roomin ylläpitoa ja myyjien auttamista ongelmatilanteissa. Hakijoilta vaaditaan aiempaa rahankäsittelykokemusta.', [tyovoima, ylivankari]),
            ('Lipunmyynti', 'Lipunmyyntivänkärin tehtävänä on lukea kävijöiden lippuvarmenteita ja antaa heille rannekkeita. Tässä pestissä ei välttämättä tule minimi työvaatimus täyteen, joten lipunmyyntivänkäri saattaa löytää itsensä tekemästä jännittäviä yllätystehtäviä.', [tyovoima, ylivankari]),
            ('Tekniikka', 'Tekniikkavänkärit huolehtivat tapahtuman aikana teknologian toimivuudesta. Työtehtävät ovat hyvin monipuolisia jatkojohtojen toimittamisesta ohjelmansalien äänentoistosta huolehtimiseen, joten tietotekniikan laaja perustuntemus on valinnan edellytyksenä.', [tyovoima, ylivankari]),
            ('Cosplay', 'Cosplayvänkäreiden tehtävänä on huolehtia etupäässä cosplaykisaajien avustamisesta harjoitusten ja kilpailujen aikana takahuoneessa ja kulisseissa. Työ jakautuu kaikille conipäiville ja sisältää moninaista puuhaa kisaajien ohjaamisesta paikkalippujen jakamiseen ja lavasteiden kuljettamiseen.', [tyovoima, ylivankari]),
            ('Majoitus', 'Majoitusvänkärin tehtävä on vastata majoittujien turvallisuudesta. Vänkärit ottavat majoittujat vastaan, ja pitävät huolta siitä, että ketään ylimääräistä ei päästetä majoitustiloihin. Vänkärit myös kiertelevät majoitustiloja ympäri valvomassa järjestystä. Työ tehdään lattiamajoitusalueella.', [tyovoima, ylivankari]),
        ...
        AlternativeSignupForm.objects.get_or_create(
            ...
            defaults=dict(
                ...
                active_from=datetime(2015, 8, 18, 0, 0, 0, tzinfo=self.tz),
                active_until=self.event.start_time,
                signup_message='Yleinen työvoimahaku Desucon Frostbiteen ei ole enää auki. Täytä tämä lomake vain, '
