...
(function() {
  var BoolFilter, DateTimeFilter, DateTimeNullFilter, EnumFilter, M2MFilter, QueryFilter, StringFilter,
  ...
  DateTimeNullFilter = (function(_super) {
    __extends(DateTimeNullFilter, _super);

    function DateTimeNullFilter() {
      this.changeMode = __bind(this.changeMode, this);
      return DateTimeNullFilter.__super__.constructor.apply(this, arguments);
    }

    DateTimeNullFilter.prototype._createMode = function(id) {
      var options;
      options = "<select id=\"" + id + "\">\n  <option value=\"!isnull\" selected=\"selected\">Kyllä</option>\n  <option value=\"isnull\">Ei</option>\n  <option value=\"lt\">Ennen</option>\n  <option value=\"gt\">Jälkeen</option>\n</select>";
      options = $(options).on("change", (function(_this) {
        return function() {
    ...
    DateTimeNullFilter.prototype.changeMode = function() {
      var mode, options, val;
      mode = $("#" + this.id("m"));
      options = $("#" + this.id("v"));
      val = mode.val();
      ...
      if (this.debugHandler != null) {
        return eval(this.debugHandler);
      }
    };

    DateTimeNullFilter.prototype.createUi = function() {
      var output;
      output = DateTimeNullFilter.__super__.createUi.call(this, "datetime");
      $("#" + this.id("v"), output).css("display", "none");
      return output;
    };

    DateTimeNullFilter.prototype.createFilter = function() {
      var flt, format, mode, negate, value, value_flt, value_obj;
      format = this._createFormat("datetime");
      mode = $("#" + this.id("m")).val();
      negate = false;
      if (mode[0] === "!") {
        mode = mode.substr(1);
      ...
      flt = [mode, this.idName, value_flt];
      return flt;
    };

    return DateTimeNullFilter;
