...
urlpatterns = patterns('',
    ...
    url(
        r'^events/(?P<event_slug>[a-z0-9-]+)/labour/admin/mail/(?P<message_id>\d+)/preview',
        labour_admin_mail_preview_view,
        name='labour_admin_mail_preview_view'
    ),

    url(
        r'^events/(?P<event_slug>[a-z0-9-]+)/labour/admin/mail/(?P<message_id>\d+)/preview/(?P<username>[a-z0-9_]+)',
