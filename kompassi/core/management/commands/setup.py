...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        organizations = [
            ...
            if app_name.startswith("kompassi.organizations.")
        ]
        ...
        events = [
            app_name.split(".")[-1] for app_name in settings.INSTALLED_APPS if app_name.startswith("kompassi.events.")
        ]
        ...
        for pargs, opts in management_commands:
            logger.info("Setup running admin command", extra=dict(cmd=pargs[0], pargs=pargs[1:], opts=opts))
