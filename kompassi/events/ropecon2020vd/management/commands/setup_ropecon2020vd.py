...
class Setup:
    ...
    def setup_core(self):
        ...
        self.event, _unused = Event.objects.get_or_create(
            slug="ropecon2020vd",
            defaults=dict(
                name="Ropecon 2020 -verkkotapahtuma",
                name_genitive="Ropecon 2020 -verkkotapahtuman",
                name_illative="Ropecon 2020 -verkkotapahtumaan",
                name_inessive="Ropecon 2020 -verkkotapahtumassa",
    ...
    def setup_labour(self):
        ...
        if not JobCategory.objects.filter(event=self.event).exists():
            ...
            for name in [
                ...
                _job_category, _created = JobCategory.objects.get_or_create(
                    event=self.event,
                    slug=slugify(name),
                    defaults=dict(
                        name=name,
        ...
        AlternativeSignupForm.objects.get_or_create(
            ...
            defaults=dict(
                ...
                signup_form_class_path="events.ropecon2020vd.forms:OrganizerSignupForm",
                signup_extra_form_class_path="events.ropecon2020vd.forms:OrganizerSignupExtraForm",
                active_from=now(),
    ...
    def setup_programme(self):
        ...
        if not Room.objects.filter(event=self.event).exists():
            for room_name in [
                ...
                _room, created = Room.objects.get_or_create(
                    event=self.event,
                    name=room_name,
                )
        ...
        alternative_form, _unused = AlternativeProgrammeForm.objects.get_or_create(
            event=self.event,
            slug="roolipeli",
            defaults=dict(
...
class Command(BaseCommand):
    args = ""
    help = "Setup ropecon2020vd specific stuff"
