...
class Setup:
    ...
    def setup_core(self):
        ...
        self.event, _unused = Event.objects.get_or_create(
            ...
            defaults=dict(
                ...
                homepage_url="https://desucon.fi/desucon2019/",
                organization=self.organization,
                start_time=datetime(2019, 6, 14, 17, 0, 0, tzinfo=self.tz),
                end_time=datetime(2019, 6, 16, 17, 0, 0, tzinfo=self.tz),
                venue=self.venue,
            ),
        )
    ...
    def setup_labour(self):
        ...
        labour_event_meta_defaults = dict(
            ...
            work_begins=datetime(2019, 6, 14, 8, 0, tzinfo=self.tz),
            work_ends=datetime(2019, 6, 16, 21, 0, tzinfo=self.tz),
        ...
        organizer_form, _unused = AlternativeSignupForm.objects.get_or_create(
            ...
            defaults=dict(
                ...
                signup_form_class_path="events.desucon2019.forms:OrganizerSignupForm",
                signup_extra_form_class_path="events.desucon2019.forms:OrganizerSignupExtraForm",
        ...
        AlternativeSignupForm.objects.get_or_create(
            ...
            defaults=dict(
                ...
                signup_form_class_path="events.desucon2019.forms:SpecialistSignupForm",
                signup_extra_form_class_path="events.desucon2019.forms:SpecialistSignupExtraForm",
                active_from=datetime(2019, 9, 16, 0, 0, 0, tzinfo=self.tz),
                active_until=self.event.start_time,
                signup_message=(
        ...
        #         ),
        #         form_class_path='events.desucon2019.forms:AfterpartyParticipationSurvey',
    ...
    def setup_programme(self):
        ...
        for role_title in [
            ...
            _role, _unused = Role.objects.get_or_create(
        ...
        Role.objects.filter(personnel_class__event=self.event, is_default=True).exclude(
        ...
        _form, created = AlternativeProgrammeForm.objects.get_or_create(
            event=self.event,
            slug="default",
            defaults=dict(
                title="Ohjelmalomake",
        ...
        if not TimeBlock.objects.filter(event=self.event).exists():
            for start_time, end_time in [
                (
                    datetime(2019, 6, 14, 17, 0, 0, tzinfo=self.tz),
                    datetime(2019, 6, 15, 1, 0, 0, tzinfo=self.tz),
                ),
                (
                    datetime(2019, 6, 15, 9, 0, 0, tzinfo=self.tz),
                    datetime(2019, 6, 16, 1, 0, 0, tzinfo=self.tz),
                ),
                (
                    datetime(2019, 6, 16, 9, 0, 0, tzinfo=self.tz),
                    datetime(2019, 6, 16, 18, 0, 0, tzinfo=self.tz),
                ),
            ]:
...
class Command(BaseCommand):
    args = ""
    help = "Setup desucon2019 specific stuff"
