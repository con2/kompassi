...
from kompassi.forms.models.field import Field, FieldType, SingleSelectPresentation
...
class Setup:
    ...
    def setup_core(self):
        ...
        self.event, _unused = Event.objects.get_or_create(
            ...
            defaults=dict(
                ...
                homepage_url="http://2027.tracon.fi",
                organization=self.organization,
                start_time=datetime(2027, 9, 3, 16, 0, tzinfo=self.tz),
                end_time=datetime(2027, 9, 5, 18, 0, tzinfo=self.tz),
                venue=self.venue,
            ),
        )
    ...
    def setup_labour(self):
        ...
        if not JobCategory.objects.filter(event=self.event).exists():
            previous_event = Event.objects.filter(slug=f"tracon{self.event.start_time.year - 1}").first()
            if previous_event:
        ...
        AlternativeSignupForm.objects.get_or_create(
            ...
            defaults=dict(
                ...
                signup_form_class_path="events.tracon2027.forms:OrganizerSignupForm",
                signup_extra_form_class_path="events.tracon2027.forms:OrganizerSignupExtraForm",
                active_from=now(),
        ...
        LabourSurvey.objects.get_or_create(
            ...
            defaults=dict(
                ...
                form_class_path="events.tracon2027.forms:ShiftWishesSurvey",
                active_from=now(),
        ...
        LabourSurvey.objects.get_or_create(
            ...
            defaults=dict(
                ...
                form_class_path="events.tracon2027.forms:SwagSurvey",
                active_from=now(),
    ...
    def setup_station_access(self):
        ...
        for person_id, app_labels in []:
            ...
            if not person:
                logger.warning("setup_tracon2027.setup_access: Person with id %d not found, skipping", person_id)
                continue
        ...
        if person and person.user:
            ...
            logger.warning("setup_tracon2027.setup_access: Onboarding user with person id %d not found, skipping", id)
    ...
    def setup_intra(self):
        ...
        for team_name, team_slug in [
            ("Projektijohto", "projektijohto"),
            ("Johtoryhmä", "jory"),
            ("Tilat", "tilat"),
            ("Talous", "talous"),
            ("Myynti ja mainonta", "myynti"),
            ("Viestintä", "viestinta"),
            ("Grafiikka", "grafiikka"),
            ("Työvoima", "tyovoima"),
            ("Turva", "turva"),
            ("Kävijäpalvelut", "kavijapalvelut"),
            ("Tekniikka", "tekniikka"),
            ("Ohjelma", "ohjelma"),
            ("Isosali", "isosali"),
        ]:
    ...
    def setup_tickets_v2(self):
        ...
        meta, _ = TicketsV2EventMeta.objects.update_or_create(
            ...
            defaults=dict(
                ...
                provider=PaymentProvider.NONE if self.dev_tickets else PaymentProvider.PAYTRAIL,
...
class Command(BaseCommand):
    args = ""
    help = "Setup tracon2027 specific stuff"
