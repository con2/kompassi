...
from kompassi.graphql_api.language import DEFAULT_LANGUAGE, SUPPORTED_LANGUAGES
...
class Survey(models.Model):
    ...
    retention_period = models.DurationField(
        ...
        help_text=_(
            ...
            "Counted from the end of the year in which the event ends or, lacking that, in which the response was created."
        ),
    )
...
class SurveyDTO:
    ...
    def save(self, event: Event, overwrite=False) -> Survey:
        ...
        # TODO resource_stream must die
        try:
            with resource_stream(f"kompassi.events.{event.slug}", f"forms/{slug}-dimensions.yml") as f:
