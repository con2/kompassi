...
class SMSMessageOut(models.Model):
    ...
    def _send(self, *args, **kwargs):
        ...
        if not succeeded:
            raise RuntimeError(f"Max retries exceeded for SMSMessageOut(id={self.id})")

        used_credit = sum(
            float(sent["message-price"]) * 100 for sent in sent_message["messages"] if int(sent["status"]) == 0
        )
