    ...
    def as_json(self, format='default'):
        ...
        if format == 'default':
            ...
            return pick_attrs(self,
                ...
                no_language=self.ropecon2018_is_no_language if self.category.slug == 'rpg' else None,
                english_ok=self.is_english_ok if self.category.slug == 'rpg' else None,
                children_friendly=self.is_children_friendly if self.category.slug == 'rpg' else None,
                age_restricted=self.is_age_restricted if self.category.slug == 'rpg' else None,
                beginner_friendly=self.is_beginner_friendly if self.category.slug == 'rpg' else None,
                intended_for_experienced_participants=self.is_intended_for_experienced_participants if self.category.slug == 'rpg' else None,
                min_players=self.min_players if self.category.slug == 'rpg' else None,
